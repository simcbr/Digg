...
class DIGGSQLCONN:
    ...
    def createTabFriends(self):
        qStr = " create table if not exists friends (rid int not null auto_increment, mutual int, create_time int, user_id int, friend_id int, primary key(rid),"\
            +  " index user_id(user_id ASC), index friend_id(friend_id ASC) ) ";
        self.v_cur.execute(qStr)
        
        qStr = "truncate table friends"
        self.v_cur.execute(qStr)
        
        qStr = "alter table friends engine=MyISAM"
        self.v_cur.execute(qStr)        
        
        
    def createTabUsers(self):
        qStr=" create view temp as select distinct(user_id) from friends union select distinct(friend_id) from friends;"
        self.v_cur.execute(qStr)
        
        qStr=" create table if not exists users (uid int not null auto_increment, primary key (uid)) select user_id from temp"
        self.v_cur.execute(qStr)
    ...
    def createTabVotes(self):
        qStr = " create table if not exists votes (vid int not null auto_increment, vote_time int, voter_id int, story_id int, primary key(vid),"\
            +  " index voter_id(voter_id ASC), index story_id(story_id ASC) ) ";
        self.v_cur.execute(qStr)
    ...
    def createTabConnVotes(self):
        # this table created for votes whose voter exist in the friends table
        qStr = " create table if not exists connVotes (vid int not null auto_increment, vote_time int, voter_id int, story_id int, primary key(vid),"\
            +  " index voter_id(voter_id ASC), index story_id(story_id ASC) ) ";
        self.v_cur.execute(qStr)
        ...
        qStr = " insert into connVotes select vid, vote_time, voter_id, story_id from votes inner join users on votes.voter_id=users.user_id"
        self.v_cur.execute(qStr)
    ...
    def allUsers(self):
        qStr="select user_id from users"
        ret=self.outputQuery(qStr, 0, 1)
        return ret
